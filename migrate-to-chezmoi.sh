...
# Step 3: Clone the repo to chezmoi's default location (non-destructive)
echo "Cloning dotfiles repo to $CHEZMOI_DIR..."
...
git -C "$CHEZMOI_DIR" checkout "$BRANCH"
echo ""
echo "NOTE: Your homeshick repo at $DOTFILES_REPO is preserved as a backup."
echo "      Do NOT remove it until you have verified the chezmoi setup works."
...
echo "  3. Verify everything works, then manually clean up:"
echo "     rm -rf ~/.homesick/repos/dotfiles"
echo "     rm -rf ~/.homesick/repos/homeshick"
echo "     rmdir ~/.homesick/repos ~/.homesick 2>/dev/null"
